...
(async function () {
  ...
  document.getElementById('endRoundBtn').addEventListener('click', async () => {
    const p = await Store.getProgress();
    const ok = confirm(
      `دور ${UI.toPersianDigits(p.round)} با ${UI.toPersianDigits(p.completed)} آیهٔ تفسیرشده بسته می‌شود و دور ${UI.toPersianDigits(p.round + 1)} شروع می‌شود. ادامه می‌دهید؟`
    );
    if (!ok) return;
