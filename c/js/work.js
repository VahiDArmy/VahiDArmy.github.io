...
(async function () {
  ...
  // --- تعیین آیهٔ شروع: از URL، وگرنه آخرین آیه‌ای که تفسیر داشته ---
  ...
  if (params.has('surah') && params.has('ayah')) {
    ...
    if (latest) current = { surah: latest.surah, ayah: latest.ayah };
  }
  ...
  tafsirForm.addEventListener('submit', async (e) => {
    ...
    if (editingId) {
      ...
      const roundBefore = await Store.getCurrentRound();
      await Store.addTafsir({ surah: current.surah, ayah: current.ayah, content });
      tafsirContent.value = '';
      await renderTafsirsList();
      const p = await refreshProgress();
      if (p.round > roundBefore) {
        UI.toast(`🎉 دور ${UI.toPersianDigits(roundBefore)} تکمیل شد! دور ${UI.toPersianDigits(p.round)} آغاز شد`);
      } else {
