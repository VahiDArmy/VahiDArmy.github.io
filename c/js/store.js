...
const Store = (function () {
  ...
  // فهرست همهٔ تفاسیر ثبت‌شده در کل سایت (صفحه‌بندی‌شده، برای بخش «تاریخچه»)
  async function getTafsirs({ limit = 15, offset = 0 } = {}) {
    const { data, error } = await sb
      .from('tafsirs')
      .select('*')
      .order('created_at', { ascending: false })
  ...
  async function addComment({ tafsirId, guestName, content }) {
    const { data, error } = await sb
      .from('comments')
      .insert({ tafsir_id: tafsirId, guest_name: guestName || 'ناشناس', content })
      .select()
      .single();
    if (error) throw error;
    return data;
